...
actor MapProcessor[I: Any #share, O: Any #share] is (Subscriber[I] & ManagedPublisher[O])
    ...
    fun ref subscriber_manager(): SubscriberManager[O] =>
        _subscriber_manager'

    be on_next(a: I) =>
        try
            subscriber_manager().publish(action(consume a)?)
        else
            subscriber_manager().on_error(EachReactiveError)
        end

    be on_error(e: ReactiveError) =>
        subscriber_manager().on_error(e)
    ...
    be on_request(s: Subscriber[O], n: U64) =>
        ...
        match _subscription
        | let sub: Subscription iso =>
            sub.request(subscriber_manager().max_request())
        else
            // publisher.subscribe(this) has yet to complete.
            // Stash the total requsted so far so that it can
            // be requested when we receive an 'on_subscribe' event
            _requested = subscriber_manager().max_request()
            None
        end
